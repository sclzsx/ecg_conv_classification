...
def extraction(data_dir, save_dir, radius, extract_channels, extract_labels, max_idx):
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
...
# 2.实现7分类或5分类（七种分类
# （N：正常L：左束分支阻滞节拍R：右束分支阻滞节拍V：心室过早收缩A：心房性早缩|：I溶质 QRS 样伪影B：左或右束分支块）
# 3.CNN+LSMT和cnn做对比
# 4.给一段信号可以实现分类（最好实现可视化
    ...
    # save_dir = './data'
    # extraction(data_dir, save_dir, radius, extract_channels, extract_labels, max_idx)
